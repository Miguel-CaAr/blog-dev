        ...
        <div
          ...
          <template v-for="(category, index) in homeStore.listOfCategories" :key="index">
            <a
              ...
              class="cursor-pointer hover:bg-gray-400 rounded py-2 px-4 mx-2"
              >{{ category?.title }}
            </a>
          </template>
    ...
    <div class="container mx-auto flex flex-wrap py-6">
      ...
      <section class="w-full md:w-2/3 flex flex-col items-center px-3">
        <section class="w-full flex justify-end">
          <NButton
            v-if="homeStore.listOfCategories.length > 0"
            :disabled="!isFilteredByCategory"
            size="small"
            @click="removeFilters()"
            >Remover filtros
          </NButton>
        ...
        <template v-for="(post, index) in homeStore.listOfPosts" :key="index">
          <article class="flex flex-col shadow my-4">
            <a class="relative hover:opacity-75">
              <span
                v-if="homeStore.modeEdition && loginStore.userAuth.username === post?.user"
                class="absolute flex flex-col gap-2 left-2 top-2">
                <NButton @click="homeStore.updatePostModal(post)" type="info" class="rounded-full p-2"
                  ><svg
                    xmlns="http://www.w3.org/2000/svg"
                    style="width: 24px; height: 24px; fill: white"
                    viewBox="0 0 24 24">
                    <path :d="mdiPencilOutline" /></svg
                ></NButton>
                <NButton
                  @click="homeStore.openDeleteConfirmationModal(true, post)"
                  type="error"
              ...
              <NImage
                v-if="post?.miniature"
                :src="'https://res.cloudinary.com/duobjlhl9/' + post?.miniature"
                alt="Blog Cover"
                class="object-cover" />
            </a>
            <div class="bg-white flex flex-col justify-start p-6">
              ...
              <p class="text-sm pb-3">
                Por <a class="font-semibold hover:text-gray-800">{{ post?.user }}</a
                >, {{ formatDate(post?.created_at) }}
              </p>
              <a class="pb-6 ellipsis">{{ post?.content }}</a>
              <a @click="goToPost(post?.title)" class="pt-2 cursor-pointer uppercase text-gray-800 hover:text-black"
        ...
        <!-- !----------Pagination------------ -->
        <div v-if="numPages" class="flex items-center py-8 gap-2">
          <a
            v-for="(number, index) in numPages"
            :key="index"
            @click="changePage(number)"
            :class="{
              'bg-blue-800 text-white': homeStore.currentPage === number,
              'text-gray-800 text-black': homeStore.currentPage !== number,
            }"
      ...
      <aside class="w-full md:w-1/3 flex flex-col items-center px-3 relative">
        <div class="w-full sticky top-0">
          <div class="w-full bg-white shadow flex flex-col my-4 p-6">
            ...
            <a
              href="https://www.linkedin.com/in/miguel-castro-1b99502b8/"
          ...
          <div class="w-full bg-white shadow flex flex-col my-4 p-6">
            ...
            <!-- Button github -->
            <a
              href="https://github.com/Miguel-CaAr"
...
// -----------COMPONENTS----------//
import { NImage, NButton, NSkeleton, NEmpty } from "naive-ui";
...
const numPages = computed(() => {
  const numberOfPosts = homeStore.numberOfPosts;
  const postsByPage = 10;
  const totalPages = Math.ceil(numberOfPosts / postsByPage);
  return totalPages;
});

const changePage = (page) => {
  const category = homeStore.category;
  if (category) {
    useHome.getAllPostsByCategoryByPage(category?.title, page);
  } else {
    useHome.getAllPostsByPage(page);
  }
};
...
const filterByCategory = (category) => {
  homeStore.category = category;
  useHome.getAllPostsByCategory(category?.title);
  handleFilterCategory(true);
};

onMounted(async () => {
  await Promise.all([useHome.getAllCategories(), useHome.getAllPosts(), useHome.getComments()]);
  homeStore.resetCurrentPage();
});
</script>
