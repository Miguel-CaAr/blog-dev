<template>
  <div class="p-10 xs:p-0 mx-auto md:w-full md:max-w-md">
    <h1 class="font-bold text-center text-2xl mb-5">Registrarse</h1>
    <div class="bg-white shadow w-full rounded-lg divide-y divide-gray-200">
      <div class="px-5 py-7">
        <NFormItem label="Correo">
          <NInput
            v-model:value="registerStore.form.email"
            type="email"
            placeholder="Ingrese correo"
            class="border rounded-lg px-3 py-2 mt-1 text-sm w-full" />
        </NFormItem>
        <NFormItem label="Nombre de usuario">
          <NInput
            v-model:value="registerStore.form.username"
            type="text"
        ...
        <NFormItem label="Contraseña">
          <NInput
            @keydown.enter.prevent="useRegister.registerUser(registerStore.form)"
            v-model:value="registerStore.form.password"
            type="password"
            placeholder="Ingrese contraseña"
            class="border rounded-lg px-3 py-2 mt-1 text-sm w-full" />
        </NFormItem>
        <NButton
          @click="useRegister.registerUser(registerStore.form)"
          size="large"
          type="info"
        ...
        <div class="grid grid-cols-2 gap-1">
          ...
          <div class="text-center sm:text-right whitespace-nowrap">
            <button
              ...
              <span @click="onToggleShowLogin(true)" class="inline-block ml-1">Iniciar sesion</span>
            </button>
          </div>
        </div>
      </div>
