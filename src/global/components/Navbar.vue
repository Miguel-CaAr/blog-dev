<template>
  <header>
    <nav class="flex flex-wrap items-center justify-between w-full py-4 md:py-0 px-4 bg-blue-800 z-10 shadow">
      <section class="flex gap-3 cursor-pointer" @click="goHomePage">
        ...
        <span className="text-white text-2xl font-bold">MI-NEWS</span>
      </section>
      ...
      <section ref="menu" class="hidden w-full md:flex md:items-center md:w-auto">
        <ul class="pt-4 text-base text-white md:flex md:justify-between md:pt-0 gap-3">
          <li class="flex md:my-3 my-2 justify-center items-center gap-2">
            <NButton
              @click="homeStore.toggleModeEdition(!homeStore.modeEdition)"
              v-if="loginStore.userAuth.isAuth"
              :type="homeStore.modeEdition ? 'warning' : 'info'"
          ...
          <li class="flex md:my-3 my-2 justify-center items-center gap-2">
            ...
            <template v-else>
              <div class="flex gap-3 w-full">
                <NButton
                  @click="homeStore.openCreatePostModal()"
                  type="success"
                  class="md:p-4 flex justify-center w-1/2">
                  Crear publicacion
                </NButton>
                <NButton @click="loginStore.logout()" type="error" class="md:p-4 flex justify-center w-1/2">
                  Cerrar sesion
                </NButton>
              </div>
            </template>
